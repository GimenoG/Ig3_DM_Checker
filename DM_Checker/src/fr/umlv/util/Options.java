...
public class Options {
	...
	public void createOptions() throws JSAPException{
		...
		FlaggedOption optForceI = (FlaggedOption) new FlaggedOption("optForceI").setList(true).setShortFlag('I').setLongFlag("forceinterdit").setListSeparator(',').setAllowMultipleDeclarations(true).setLongFlag(JSAP.NO_LONGFLAG).setHelp("Check the lack of the file named <param> in the repository. The head directory isn't concern by is option.\nUse : -I <regex name> or --interdit <regex name>");
		FlaggedOption optForceX = (FlaggedOption) new FlaggedOption("optForceX").setList(true).setShortFlag('X').setLongFlag("forceexiste").setListSeparator(',').setAllowMultipleDeclarations(true).setLongFlag(JSAP.NO_LONGFLAG).setHelp("Check if the folder name in parameter already existe in the destination directory. If it's true the folder won't be treaty");
		FlaggedOption optForceB = (FlaggedOption) new FlaggedOption("optForceB").setList(true).setShortFlag('B').setLongFlag("forcebeginsWith").setListSeparator(',').setAllowMultipleDeclarations(true).setLongFlag(JSAP.NO_LONGFLAG).setHelp("Doesn't work with the files which start with the paramter : -b <start string to bansih> or --beginswith <start string to banish>");
		FlaggedOption optForceE = (FlaggedOption) new FlaggedOption("optForceE").setList(true).setShortFlag('E').setLongFlag("forceendsWith").setListSeparator(',').setAllowMultipleDeclarations(true).setLongFlag(JSAP.NO_LONGFLAG).setHelp("Doesn't work with the files which end with the paramter : -e <end string to bansih> or --endswith <end string to banish>");
		Switch optForceO = (Switch)(new Switch("optForceO").setShortFlag('O').setLongFlag("forceonetop").setLongFlag(JSAP.NO_LONGFLAG).setHelp("Require only one folder at the top"));
